...
const Calendar = () => {
  ...
  */
   ...
   * 特定の日付の取引データを取得する関数
   */
  const getTransactionForDate = (year: number, month: number, date: number): Transaction | undefined => {
    return transactions.find((t: Transaction) => 
      t.year === year && 
      t.month === month && 
      t.date === date
    );
  ...
  */
  const renderCalendarDays = (): JSX.Element[] => {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      const hasIncome: boolean = transaction !== undefined && transaction.income > 0;
      const hasExpense: boolean = transaction !== undefined && transaction.expense > 0;
            ...
            {dayNames.map((day, index) => (
              <div 
                ...
                className={`p-2 text-center text-xs font-medium border-r border-gray-200 ${
                  index === 0 ? 'text-red-500' :    // 日曜日は赤色
                  index === 6 ? 'text-blue-500' :   // 土曜日は青色
                  'text-gray-700'                   // 平日は灰色
                }`}
              >
                {day}
