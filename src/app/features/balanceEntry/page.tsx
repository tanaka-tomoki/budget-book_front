...
import { BudgetDetailInfo, emptyBudgetDetailInfo } from "@/lib/intarFaces/budgetDetailInfo";
...
const BalanceEntry = () => {
  ...
  // 選択されたデータ
  const [selectedFormData, setSelectedFormData] = useState<BudgetDetailInfo>(emptyBudgetDetailInfo);
  ...
  const saveEditData = (formData: BudgetDetailInfo) => {
    ...
    const existingKey = Object.keys(data).find(key => data[key].budgetDetailId == formData.budgetDetailId);
    // 内容が空の場合、アイコンの名前を設定
