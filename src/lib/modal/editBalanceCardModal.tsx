...
const EditBalanceCardModal: React.FC<EditBalanceCardModalProps> = ({
  onClose, onClickSaveButton, budgetDetailInfo
}) => {
  // エラーハンドル
  ...
  const [editType, setEditType] = useState<BalanceType>(formData.balanceType || 'income');
  const defaultIcon = IconManager.getDefault();
  const [iconInfo, setIconInfo] = useState(IconManager.getById(budgetDetailInfo.categoryId) || defaultIcon);
  ...
  return (
    <div className='modal-content edit-balance-card-modal'>
      <IconEditModal
        ...
        onIconSelect={(selectedIconInfo) => handleEditChange('categoryId')(selectedIconInfo)}
      ...
      <div className='mt-6'>
        ...
        <LocalizationProvider dateAdapter={AdapterDateFns} adapterLocale={ja}>
          ...
          >
            <DatePicker
              value={
                formData.updateAt ? parseDate(formData.updateAt) : null
              }
              format='yyyy/M/d'
